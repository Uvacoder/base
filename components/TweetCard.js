...
export default function Tweet ({
  ...
  const LinkID = Link.match(IDregex)
  const UsernameRegex = new RegExp('(?:https?:\\/\\/)?(?:www\\.)?twitter\\.com\\/(?:#!\\/)?@?([^\\/\\?\\s]*)', 'g')
  const Username = UsernameRegex.exec(Link)
  const authorUrl = `https://twitter.com/${Username[1]}`
  const likeUrl = `https://twitter.com/intent/like?tweet_id=${LinkID}`
  const retweetUrl = `https://twitter.com/intent/retweet?tweet_id=${LinkID}`
  const replyUrl = `https://twitter.com/intent/tweet?in_reply_to=${LinkID}`
  const tweetUrl = Link
    ...
    <div className=" my-3 w-full max-w-3xl rounded-md border border-gray-200 bg-white px-4 py-4 shadow-lg dark:border-zinc-900 dark:bg-zinc-900">
      <div className="flex items-center">
        ...
        >
          ...
          <span className="!text-gray-500 hover:!underline mt-0.5" title={`@${Username[1]}`}>
            @{Username[1]}
          </span>
        </a>
      ...
      <a href={tweetUrl} target="_blank" rel="noopener noreferrer">
        <div className="mt-4 mb-2 whitespace-pre-wrap leading-normal !text-gray-700 dark:!text-gray-200">
          {formattedTweet}
        </div>
      </a>
      <a
        ...
        rel="noopener noreferrer"
      >
        {Type}{' | '}<time title={`Time Posted: ${createdAt.toUTCString()}`} dateTime={createdAt.toISOString()}>
      ...
      <div className="mt-2 flex text-sm !text-gray-700 dark:!text-gray-300">
        ...
        <a
          className="mr-4 flex items-center !text-gray-500 transition hover:!text-green-400 hover:!underline"
          href={retweetUrl}
          target="_blank"
          rel="noopener noreferrer"
        >
        ...
        <a
          className="flex items-center !text-gray-500 transition hover:!text-red-500 hover:!underline"
          href={likeUrl}
          target="_blank"
          rel="noopener noreferrer"
        >
